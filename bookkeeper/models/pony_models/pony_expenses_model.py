...
from bookkeeper.models.abstract_expense_model import AbstractExpense, AbstractCategory, \
    AbstractExpensesModel, ExpenseConstraint, ConstraintType, ExpenseField
from bookkeeper.models.abstract_model import AbstractModel
from bookkeeper.exceptions import PrimaryKeyAssignmentError, ConstraintError, NoDataError
...
class PonyExpensesModel(AbstractExpensesModel):
    ...
    @db_session
    def add_expense(self, amount: float, category: AbstractCategory, 
                    expense_date: typing.Optional[datetime] = None,
                    comment: typing.Optional[str] = None) -> PonyExpense:
        raw_kwargs = {"expense_date": expense_date, "comment": comment}
        kwargs = {kwarg: raw_kwargs[kwarg] for kwarg in raw_kwargs if 
                  raw_kwargs[kwarg] is not None}
        new_expense = self.db.Expense(amount=amount, 
                                      category=self.db.Category[category.id], 
                                      **kwargs)
        new_expense.flush()
    ...
    # @db_session
    # def get_expense_by_id(self, id: int) -> PonyExpense:
    #     expense= self.db.Expense[id]
    #     atrs = expense.to_dict()
    #     atrs["category"] = self.model.category_model.get_category_by_id(atrs["category"])
    ...
    def get_expenses_by_ids(self, ids: list[int]) -> list[PonyExpense]:
        ...
        for id in ids:
            try:
                ...
                fail = True
        if fail:
            raise NoDataError("There is no expenses in database for one \
                              or more ids provided", result)
        return result
    
    @db_session
    def get_expenses_by_constraints(self, 
                                    constraints: list[ExpenseConstraint],
                                    max_num: typing.Optional[int] = None) -> list[PonyExpense]:
        ...
        for c in constraints:
            if c.expense_field == ExpenseField.category:
                ...
                def check(f, expr, lfl, eqfl, gfl): 
                    return ((f < expr and lfl) or (f == expr and eqfl) or (f > expr and gfl))
                flags = [bool(c.constraint_type & ConstraintType.less),
                          bool(c.constraint_type & ConstraintType.equal),
                          bool(c.constraint_type & ConstraintType.greater)]
                query = query.where(lambda e: check(getattr(e, c.expense_field), c.expression, flags[0], flags[1], flags[2]))
    ...
    def _validate_constraint(self, constraint: ExpenseConstraint) -> bool:
        if constraint.constraint_type != ConstraintType.equal and \
            (constraint.expense_field == ExpenseField.category or \
             constraint.expense_field == ExpenseField.comment):
            return False
        return True

    @db_session
    def set_attributes(self, expense: PonyExpense, 
                       attr_dict: dict[ExpenseField, typing.Any]) -> None:
        if "id" in attr_dict.keys():
            raise(PrimaryKeyAssignmentError("Attribute \"id\" of PonyExpense \
        ...
        if ExpenseField.category in attr_dict.keys():
            ...
            attr_dict[ExpenseField.category] = \
                self.db.Category[attr_dict[ExpenseField.category].id]
