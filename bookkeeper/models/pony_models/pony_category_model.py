...
from bookkeeper.models.abstract_category_model import AbstractCategory, \
    AbstractCategoryModel, CategoryDeletePolicy, CategoryField, ExpensesHandlingPolicy
...
class PonyCategory(AbstractCategory):
    ...
    def delete(self, 
            ...
            expense_handling: ExpensesHandlingPolicy = ExpensesHandlingPolicy.delete) -> \
                tuple[int, int]:
        return self.model.delete_category(self, 
...
class PonyCategoryModel(AbstractCategoryModel):
    ...
    def add_category(self, name: str, 
                     parent: typing.Optional[AbstractCategory] = None) -> \
                                                                PonyCategory:
        ...
        new_category.flush()
        # return PonyCategory(model=self, id=new_category.id, name=name)
        return self._form_ponycat(new_category)

    @db_session
    def delete_category(self, cat: PonyCategory, 
            children_policy: CategoryDeletePolicy = CategoryDeletePolicy.delete,
            expense_handling: ExpensesHandlingPolicy = ExpensesHandlingPolicy.delete) -> \
                tuple[int, int]:
        cat_to_del = self.db.Category[cat.id]
        (cat_touched, exp_touched) = self._del_cat(cat_to_del, 
                                                   children_policy=children_policy,
                                                   expense_handling=expense_handling,
    ...
    @db_session
    def _del_cat(self, cat_to_del: Self.db.Category, 
            children_policy: CategoryDeletePolicy = CategoryDeletePolicy.delete,
            expense_handling: ExpensesHandlingPolicy = ExpensesHandlingPolicy.delete,
            parent_for_exps: typing.Optinal[Self.db.Category] = None) -> tuple[int, int]:
        cat_touched = 0
        exp_touched = 0
    ...
    def get_parent(self, category: PonyCategory) -> typing.Optional[PonyCategory]:
        ...
        if parent_category is None:
            return None
        # return PonyCategory(model=self, id=parent_category.id, name=parent_category.name)
    ...
    def update_category(self, 
                        cat: PonyCategory, 
                        upd_data: dict[CategoryField, typing.Any]) -> PonyCategory:
        cat_to_upd = self.db.Category[cat.id]
        if CategoryField.parent in upd_data:
            if upd_data[CategoryField.parent] is not None:
                upd_data[CategoryField.parent] = self.db.Category[upd_data[CategoryField.parent]]
    ...
    def get_categories_by_ids(self, ids: list[int]) -> list[PonyCategory]:
        ...
        for id in ids:
            try:
                ...
                # result.append(
                #     PonyCategory(model=self, 
                #                 **loaded_cat.to_dict(only=['id', 'name'])))
            except ObjectNotFound:
                fail = True
        if fail:
            raise NoDataError(("There is no categories in database for one "
    ...
    def get_root_categories(self) -> list[PonyCategory]:
        ...
        # result = [PonyCategory(model=self, **rcat.to_dict(only=['id', 'name'])) for 
        #                 rcat in root_cats]
    ...
    def get_whole_subtree(
            self, category: typing.Optional[PonyCategory] = None) -> list[PonyCategory]:
    ...
    def _form_ponycat(self, cat: Self.db.Category) -> PonyCategory:
        return PonyCategory(model=self, 
                            id=cat.id, 
                            name=cat.name)