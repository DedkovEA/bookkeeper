...
class ExpenseAddWidget(QtWidgets.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        blayout.addWidget(QtWidgets.QLabel("Amount"), 0, 0, 
    ...
    def _invoke_expense_addition(self) -> None:
        ...
        if self.category_selection.currentItem() is self.category_selection._root:
            ...
            return
        try:
            self._expense_add_handler(self.amount.text(), 
    ...
    # @QtCore.Slot()
    # def _add_expense(self, amount: int, category: int, 
    #                  expense_date: Optional[datetime] = None, comment: str = "") -> None:
    #     ...

    # register handlers for functionality
    def register_expense_add_handler(
            self, 
            handler: Callable[[str, int, Optional[str], str], None]) -> None:
