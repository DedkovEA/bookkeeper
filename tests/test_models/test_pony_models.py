...
from bookkeeper.models.abstract_category_model import CategoryDeletePolicy, \
                                                      ExpensesHandlingPolicy
from bookkeeper.models.abstract_expense_model import ConstraintType, ExpenseConstraint, ExpenseField
from bookkeeper.models.pony_models.pony_model import PonyModel
from bookkeeper.models.pony_models.pony_category_model import PonyCategoryModel, PonyCategory
from bookkeeper.models.pony_models.pony_expenses_model import PonyExpensesModel, PonyExpense
...
def some_expenses(exp_model, some_cats):
    return [exp_model.add_expense(i, some_cats[1], comment=f"Expense {i}") 
            for i in range(5)]

@pytest.fixture(scope='module')
def not_delete_expense(exp_model, some_cats):
    return exp_model.add_expense(777, some_cats[3], comment=f"Expense wich we preserve") 
...
def expenses_for_test(exp_model, cat_to_find):
    return [exp_model.add_expense(700700+i, cat_to_find, 
...
def cat_tree(cat_model):
    ...
    c1 = [cat_model.add_category(f"child of ROOT num {i}", parent=c05) for i in range(5)]
    c2 = [[cat_model.add_category(f"child of ROOT_num_{j} child num {i}", parent=c1[j]) 
           for i in range(5)] 
           for j in range(5)]
    return (c0,c05,c1,c2)
...
class TestCategory:
    ...
    def test_delete_category_with_children_and_exps(self, cat_model, 
                                                    cat_tree, exp_model):
        (c0, c05, c1, c2) = cat_tree
        exp_ids = []
        for cs in c2:
    ...
    def test_delete_category_with_children_moved_exps(self, cat_model, 
                                                      cat_tree, exp_model):
        (c0, c05, c1, c2) = cat_tree
        exp_ids = []
        for cs in c2:
            for c in cs:
                ...
                exp_ids.append(exp.id)
        c05id = c05.id
        (n1, n2) = c05.delete(CategoryDeletePolicy.delete,
                              ExpensesHandlingPolicy.move)
        assert c05.id is None
        assert c05.name == "DELETED"
        assert n1 == 31
        assert n2 == 25
...
class TestExpense:
    def test_add_expense(self, exp_model, some_cats):
        ...
        timing1 = datetime.now()
        e2 = exp_model.add_expense(321, some_cats[1], 
                                   datetime(2024, 4, 5, 15, 13),
                                   # datetime(2024, 4, 5, 15, 14, 12),
                                   "I love python")  # full addition
        timing2 = datetime.now()
    ...
    def test_set_attributes_from_expense(self, exp_model, some_cats):
        e1 = exp_model.add_expense(321, some_cats[1], 
                                   # datetime(2024, 4, 5, 15, 13),
                                   datetime(2024, 4, 5, 15, 14, 12),
                                   "I love python")  # full addition
    ...
    def test_set_attributes(self, exp_model, some_cats):
        e1 = exp_model.add_expense(321, some_cats[1], 
                                   ...
                                   "I love python")  # full addition
        # Multiple attr set
        exp_model.set_attributes(e1, {"amount" : 12, "category" : some_cats[3],
    ...
    def test_set_attributes_fails_with_id(self, exp_model, some_expenses, some_cats):
        with pytest.raises(PrimaryKeyAssignmentError):
            exp_model.set_attributes(some_expenses[1], 
    ...
    def test_validate_constrain(self, exp_model):
        ...
        right_constr2 = ExpenseConstraint(ExpenseField.comment, ConstraintType.equal, "s")
        ...
        assert exp_model._validate_constraint(right_constr) == True
        assert exp_model._validate_constraint(right_constr2) == True
        assert exp_model._validate_constraint(fail_constr) == False
        assert exp_model._validate_constraint(fail_constr2) == False

    def test_get_expenses_by_constraint_num(self, exp_model, 
                                            expenses_for_test, some_cats):
        # find one_arg with amount
        assert set(exp_model.get_expenses_by_constraints(
               [ExpenseConstraint(ExpenseField.amount, 
                                  ConstraintType.geq, 
                                  700702)])) == \
        ...
        assert set(exp_model.get_expenses_by_constraints(
               [ExpenseConstraint(ExpenseField.expense_date, 
                                  ConstraintType.less, 
    ...
    def test_get_expenses_by_constraint_comment(self, exp_model, 
                                                expenses_for_test):
        # find one_arg with comment
        assert set(exp_model.get_expenses_by_constraints(
               [ExpenseConstraint(ExpenseField.comment, 
                                  ConstraintType.equal, 
                                  "Expense to find 2")])) == set([expenses_for_test[2]])

    def test_get_expenses_by_constraint_comment_fails(self, exp_model, 
                                                      expenses_for_test):
    ...
    def test_get_expenses_by_constraint_category(self, exp_model, 
                                                 expenses_for_test, cat_to_find):
        # find one_arg with category
        assert set(exp_model.get_expenses_by_constraints(
               [ExpenseConstraint(ExpenseField.category, 
                                  ConstraintType.equal, 
                                  cat_to_find)])) == set(expenses_for_test)

    def test_get_expenses_by_constraint_category_fails(self, exp_model, 
                                                 expenses_for_test, cat_to_find):
    ...
    def test_get_expenses_by_constraint_many(self, exp_model, 
                                             expenses_for_test, cat_to_find):
